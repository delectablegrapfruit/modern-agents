...
final class CoverLayoutTests: XCTestCase {
    ...
    func testCoverStyleAndReadingCountsRoundTrip() throws {
        ...
        XCTAssertEqual(settings.home.visible, [.continueReading, .goals, .activity, .recentlyAdded], "Home starts simple")
        XCTAssertEqual(settings.home.size(of: .goals), .medium)
        XCTAssertEqual(settings.home.size(of: .continueReading), .large)
        XCTAssertEqual(settings.home.size(of: .activity), .medium)
        XCTAssertEqual(settings.home.size(of: .recentlyAdded), .extraLarge)
        ...
        XCTAssertTrue(legacy.home.isShown(.continueReading))
        ...
        chosen.home.setSize(.large, for: .statistics)
        chosen.home.setSize(.extraLarge, for: .calendar)
        let decoded = try JSONDecoder().decode(Settings.self, from: JSONEncoder().encode(chosen))
        XCTAssertEqual(decoded.shelves["collection:abc"]?.sort, .length)
        XCTAssertEqual(decoded.shelves["collection:abc"]?.grouping, .genre)
        XCTAssertEqual(Array(decoded.home.elements.prefix(3)), [HomeElement.statistics, .goals, .continueReading])
        XCTAssertEqual(decoded.home.hidden, [HomeElement.activity])
        XCTAssertEqual(decoded.home.size(of: .statistics), .large)
        XCTAssertEqual(decoded.home.size(of: .calendar), .small, "a size the widget cannot take is not kept")
        let odd = try JSONDecoder().decode(HomeSettings.self, from: Data(#"{"order": ["goals", "somethingNew"], "hidden": ["gone"]}"#.utf8))
        XCTAssertEqual(odd.order, [HomeElement.goals])
...
final class HomeDataTests: XCTestCase {
    func testGenresFromSubjects() {
        ...
        XCTAssertEqual(Genres.genres(for: ["Smartphone repair"]), [], "a word inside another word is not a match")
        XCTAssertEqual(Genres.genres(for: ["Natural history"]), ["Science & Nature"], "a phrase uses up its words")
        XCTAssertEqual(Genres.genres(for: ["Historical fiction", "Political science"]), ["Historical Fiction", "Politics & Society"])
    ...
    func testGoalPeriodsAndTotals() throws {
        ...
        XCTAssertEqual(stats.totals(in: .month, containing: date, calendar: calendar).seconds, 900, "the 29th of August is not this month")
    ...
    func testHomeSettingsMoves() {
        var home = HomeSettings(hidden: [])
        home.move(fromOffsets: IndexSet(integer: 0), toOffset: 3)
        XCTAssertEqual(Array(home.elements.prefix(3)), [HomeElement.pickUpAgain, .goals, .continueReading])
        home.setShown(.calendar, false)
        home.setShown(.calendar, false)
        XCTAssertEqual(home.hidden, [HomeElement.calendar], "hiding twice hides once")
        XCTAssertFalse(home.visible.contains(.calendar))
        home.setShown(.calendar, true)
        XCTAssertTrue(home.hidden.isEmpty)
        XCTAssertEqual(WidgetSize.extraLarge.span, GridSpan(columns: 4, rows: 2))
        XCTAssertEqual(HomeElement.calendar.families, [.small, .medium, .large])
        home.setSize(.medium, for: .goals)
        XCTAssertTrue(home.sizes.isEmpty, "the usual size is not written down")
        home.setSize(.small, for: .goals)
        XCTAssertEqual(home.size(of: .goals), .small)
