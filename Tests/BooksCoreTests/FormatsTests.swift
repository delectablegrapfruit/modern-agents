...
final class CoverLayoutTests: XCTestCase {
    ...
    func testCoverStyleAndReadingCountsRoundTrip() throws {
        ...
        chosen.home.setSize(.wide, for: .statistics)
        chosen.home.setSize(.wide, for: .calendar)
        ...
        XCTAssertEqual(decoded.home.size(of: .statistics), .wide)
        XCTAssertEqual(decoded.home.size(of: .calendar), .medium, "a size the widget cannot take is not kept")
...
final class HomeDataTests: XCTestCase {
    ...
    func testHomeSettingsMoves() {
        ...
        XCTAssertEqual(WidgetSize.wide.units, 4)
        XCTAssertTrue(HomeElement.calendar.sizes == [.medium])
