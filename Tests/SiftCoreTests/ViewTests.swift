...
final class WindowGuardTests: XCTestCase {
    ...
    func testLookScriptCarriesRulesAndKnownWindows() {
        ...
        XCTAssertTrue(script.contains("if current view of w is not flow view then\nset current view of w to flow view\nset did to true"), "Finder's name for the gallery view; a change is counted")
        XCTAssertFalse(script.contains("gallery view"), "not a term Finder knows")
        XCTAssertTrue(script.contains("if arrangement of o is not arranged by kind then\nset arrangement of o to arranged by kind"), "set only when it differs: Finder records every change")
        XCTAssertTrue(script.contains("if name of sort column of o is not modification date column then\nset sort column of o to modification date column"), "a column answers as an object; its name is the enumerator")
        XCTAssertTrue(script.contains("set sort direction of sort column of o to reversed"))
        XCTAssertTrue(script.contains("set icon size of o to large icon"))
        XCTAssertTrue(script.contains("if did then\nset end of out to \"+ \" & (r as text) & \" \" & entry\nelse\nset end of out to \"= \" & entry"), "only a window that was changed counts as set")
        ...
        XCTAssertTrue(script.contains("else\nset r to 2"), "the default is the last rule")
