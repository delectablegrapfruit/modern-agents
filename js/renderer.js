// WebGL2 vector renderer: every shape is a batch of soft-edged line quads drawn additively into an HDR
// target, then bloomed (two blur octaves) and composited to the screen.
'use strict';
(function () {
  const GW = window.GW;
...
out vec4 o;
void main() {
  vec3 c = texture(u_scene, v_uv).rgb + texture(u_b1, v_uv).rgb * u_k1 + texture(u_b2, v_uv).rgb * u_k2;
