/* Application bootstrap: appearance, toolbar, sidebar, drag & drop import, keyboard shortcuts, PWA plumbing. */
(function (global) {
  'use strict';
  ...
  const App = {
    ...
    setIcons() {
      ...
      $('#sort-btn .tb-popup-chevrons').innerHTML = Icons.icon('chevronUp', { size: 12, stroke: 2.4 }) + Icons.icon('chevronDown', { size: 12, stroke: 2.4 });
    ...
    wireToolbar() {
      ...
      $('#sort-btn').addEventListener('click', e => UI.menu([
        { label: 'Recent', checked: Settings.get('sort') === 'recent', action: () => { Settings.set('sort', 'recent'); Library.render(); } },
        { label: 'Title', checked: Settings.get('sort') === 'title', action: () => { Settings.set('sort', 'title'); Library.render(); } },
        { label: 'Author', checked: Settings.get('sort') === 'author', action: () => { Settings.set('sort', 'author'); Library.render(); } },
      ...
      search.addEventListener('keydown', e => { if (e.key === 'Escape') { search.value = ''; Library.search = ''; Library.renderView(); search.blur(); } if (e.key === 'Enter') { const first = $('#view [data-id]'); if (first) { first.focus(); first.click(); } } });
      ...
      $('#tl-zoom').addEventListener('click', () => { if (document.fullscreenElement) document.exitFullscreen().catch(() => {}); else document.documentElement.requestFullscreen?.().catch(() => {}); });
      $('#tl-close').addEventListener('click', () => { if (matchMedia('(display-mode: standalone)').matches) window.close(); });
    },
    toggleSidebar() { const c = $('#app').classList.toggle('sidebar-collapsed'); Settings.set('sidebarCollapsed', c); $('#btn-sidebar').title = (c ? 'Show Sidebar' : 'Hide Sidebar') + ' (⌃⌘S)'; },
    moreMenu(anchor) {
      ...
      UI.menu([
        ...
        { label: 'Add Sample Books', icon: 'sample', action: () => Library.installSamples() },
        { separator: true },
    ...
    wireSidebar() {
      // collection drop targets are wired during render; nothing else to do here
    },
    wireDragDrop() {
      const content = $('#content'), overlay = $('#drop-overlay');
      let depth = 0;
      ...
      content.addEventListener('contextmenu', e => {
        if (e.target.closest('[data-id], .continue-card, input, textarea')) return;
    ...
    wireKeys() {
      document.addEventListener('keydown', e => {
        ...
        if (mod && e.key.toLowerCase() === 'n' && e.shiftKey) { e.preventDefault(); Library.createCollection(); return; }
    ...
    showShortcuts() {
      const m = U.modKey;
      const groups = [
        ['Library', [['Add to Library…', m + ' O'], ['Search library', m + ' F'], ['Open selected book', 'Return'], ['Get Info', 'Space'], ['Delete selected', '⌫'], ['Select all', m + ' A'], ['Toggle sidebar', '⌃' + m + ' S'], ['Back / Forward', m + ' [  ' + m + ' ]'], ['Home / All', m + ' 1  ' + m + ' 2']]],
        ...
        ['Mouse & Trackpad', [['Turn page', 'Scroll wheel / two-finger scroll'], ['Turn page (horizontal)', 'Shift + wheel / two-finger swipe'], ['Text size', m + ' + scroll wheel / pinch'], ['Highlight', 'Select text, pick a colour'], ['Book actions', 'Right-click a book']]],
      ];
      const body = el('div', groups.map(([title, rows]) => el('div', { style: { marginBottom: '14px' } }, el('div.ap-section-title', title), el('div.shortcut-list', rows.map(([a, b]) => el('div.row', el('span', a), el('span', b)))))));
    ...
    async showStorage() {
      ...
      const body = el('div', el('table.info-table', rows.map(([k, v]) => el('tr', el('td', k), el('td', v)))),
        el('p.msg', { style: { marginTop: '12px' } }, 'Everything is stored in this browser’s local database (IndexedDB) and never leaves your device. Clearing site data removes the library.'),
    ...
    showAbout() {
      UI.sheet({ alert: true, icon: 'bookOpen', title: 'Books (offline)', message: 'A self-contained reader modelled on Books for macOS. EPUB, PDF and plain-text files are stored locally in your browser. No account, no store, no network.\n\nSample books are public-domain texts from Project Gutenberg.' });
    },
