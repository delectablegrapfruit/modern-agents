...
itself the active app and the window key, so the windows look as they do in use. The library in them is a sample the
app lays out for the purpose: the opening chapters of fourteen public-domain classics, each with a cover drawn for
it, and Francis Bacon's essays typeset as a small book in PDF, with a running head and a page number on every page
after the first. There are four collections (Classics, Science Fiction, Gothic & Horror, Summer Reading), books part
read and finished, five months of reading history, reading goals, and highlights, notes and a bookmark in *Pride and
Prejudice*.
...
| [16-pdf-zoom-and-split.png](16-pdf-zoom-and-split.png) | The same PDF in Zoom & Split: pages cropped to their text, the running heads and page numbers cut away, and cut into screens that turn like a book's. |
...
| [21-pdf-text.png](21-pdf-text.png) | The PDF read as Text: its words reflowed into the reader's own pages and type, the running heads and page numbers left out. |
