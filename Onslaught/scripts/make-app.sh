#!/usr/bin/env bash
# Builds Onslaught.app from the Swift package (macOS only).
#   scripts/make-app.sh [debug|release]
set -euo pipefail
cd "$(dirname "$0")/.."

CONFIG="${1:-release}"
APP="build/Onslaught.app"

swift build -c "$CONFIG" --product Onslaught
BINDIR="$(swift build -c "$CONFIG" --show-bin-path)"

rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$BINDIR/Onslaught" "$APP/Contents/MacOS/Onslaught"
...
# Ad-hoc signature: enough for Gatekeeper's "Open Anyway".
