...
| Press and hold on a playing video | Plays at 2× until you let go, with a speed badge at the top of the video (as on YouTube; not applied there, where it is native). Scroll sideways while holding to change the speed in quarter steps (a short nudge is one step, a flick two), 0.25× to 8×; every hold starts at 2× again. |
...
- Press and hold: a primary-button press on a playing video that stays put for 500 ms sets `playbackRate` to 2 until release, with a "2×" badge at the top centre of the video; sideways scrolling while holding steps the rate by 0.25× per 30 px of movement, steps at least 150 ms apart and at most two per flick (continuous events without a 150 ms pause or a reversal), so a nudge is one step and a flick two (0.25× to 8×), and is not a scrub; every hold starts at 2× again. The release's click is swallowed so the player does not toggle. Presses on buttons, links, sliders and fields inside the player are left alone, as is a press that moves (a drag). Not applied on youtube.com, which has the feature natively.
