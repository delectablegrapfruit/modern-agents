...
| Action | Result |
|---|---|
| Hover a video, scroll right / left | Seek forward / back. A small slow nudge moves tenths of a second, a long quick flick about a second (0.5 s per 100 px by default, no more than about a second per wheel movement). Keep scrolling in one direction for about three seconds and the rate ramps up to four times; pause or reverse and it drops back. No dead zone, no snapping. |
...
| Press and hold on a playing video, or hold <kbd>Space</kbd> while hovering one | Plays at 2× until you let go, with a speed badge at the top of the video (as on YouTube, whose own version this replaces). Scroll sideways while holding to change the speed in quarter steps, trackball-like: the further the wheel rolls, the more steps (one per 40 px, paced 120 ms apart), 0.25× to 8×; every hold starts at 2× again. |
| Tap <kbd>Space</kbd> while hovering a video | Play / pause. Typing in a field, or a focused button, is left alone; so is the space bar when no video is under the pointer or focused. |
...
While you scrub, a thin timeline along the top of the video follows the wheel instantly (the site's own timeline only catches up when each seek completes); a faint tick marks where you started, which is where undo returns to. When you stop, it keeps following playback for a moment and fades out. It re-measures the video every frame it is visible, so it follows window resizes and fullscreen. The popup has the on/off switch, a switch for the current site, the speed, and the undo button; the options page has the rest.

### Settings

| Setting | Default | Notes |
|---|---|---|
| Speed | 0.5 s per 100 px | The base rate, whatever the video's length. 0.05 to 10 s per 100 px. |
...
| Trackball mode | off | A flick keeps the playhead rolling after the wheel stops, slowing down on its own; scrolling again takes over. |
...
- Wheel movement is converted to seconds and added to a logical position that tracks the wheel exactly, whatever the video is doing. Fractional pixels count; nothing is rounded or thresholded. Three things shape the distance a movement covers. The base rate is fine (0.5 s per 100 px), and no single wheel event moves more than about a second (beyond that only a tenth of the excess counts), so a flick never jumps far. Velocity adds a very minor percentage at high speeds only: nothing up to 1500 px/s, up to 20% at 8000 px/s. Persistence provides gross scrubbing: scrolling kept up in one direction in quick succession (gaps under 350 ms) for 3 s ramps the rate, and the per-event cap, up to 4x by 6 s; a pause or a reversal resets it, since you reverse when you overshoot, which is when you want fine control. The timeline shows the current multiplier when it is not 1.
...
- The timeline lives in a closed shadow root and uses the Popover API, so it renders in the top layer even above fullscreen players. It draws the logical position (where the wheel is), not the video's `currentTime`, so it never lags a seek; it re-measures the video and redraws once per animation frame while visible (reads before writes), follows `currentTime` for 1.2 s after the gesture, then fades over 0.5 s.
- Press and hold: a primary-button press on a playing video that stays put for 500 ms sets `playbackRate` to 2 until release, with a "2×" badge at the top centre of the video; sideways scrolling while holding steps the rate by 0.25× per 40 px of movement, accumulated continuously and paced at least 120 ms apart with at most one step's worth carried over, so the speed follows how far the wheel rolls (0.25× to 8×), and is not a scrub; every hold starts at 2× again. The release's click is swallowed so the player does not toggle. Presses on buttons, links, sliders and fields inside the player are left alone, as is a press that moves (a drag).
- The space bar does the same. It is taken over on its first `keydown` (the page's own action, a scroll or a play/pause toggle, happens right then, so the decision cannot wait for the hold) when a video is under the pointer or inside the focused element (a player that took focus when clicked), and never while typing, on a button, or during a scrub. Held for 500 ms it speeds the video up like a press; released earlier it toggles play/pause, as the player would have. On a paused video it just plays.
- On youtube.com (and youtube-nocookie.com, youtu.be), which has this feature natively, the extension replaces it rather than doubling it. A press on the video is owned outright: `pointerdown`, `mousedown`, `pointerup`, `mouseup` and the click are all stopped before the page's handlers see them (the site's click-to-pause is handled on the release, along with its hold, so the events cannot be split), and the extension does what the press would have done: a release before the delay toggles playback, a hold speeds up, a drag does neither. A double-click still reaches the page (`dblclick` is dispatched whatever became of the clicks), so its fullscreen toggle keeps working, and presses on the progress bar and the controls are left alone. The space bar's `keydown`/`keyup` are stopped the same way, and, as on the site itself, the key also reaches the main player from anywhere on the page: the playing video covering at least a fifth of the viewport, so inline previews do not count.
- Trackball mode: when the wheel stops after a flick (at least 1.5 s of video per second), the playhead keeps moving at the last velocity, decaying with a 0.4 s time constant until it drops under 0.25 s/s, driven by a frame loop through the same seek scheduler; any wheel event cancels it and takes over, and playback resumes only once it has come to rest.
...
The tests serve a fixture page (plain video, video under an overlay, `pointer-events: none` video, open and closed shadow DOM players, iframe player, a `preload="none"` video, a clip with 10 s keyframe spacing to make seeks expensive; plus a page with a YouTube-like native hold, served as www.youtube.com through request interception), dispatch real wheel events through the Chrome DevTools Protocol, and check `currentTime`, page scroll position, the pixels of the frame on screen, how progressively frames appear during a fast burst, the timeline, undo, and the popup and options pages. The fixture clips encode the time in every frame (hue = second, bar width = frame within the second) so tests can verify what is actually on screen.
