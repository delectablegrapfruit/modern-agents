...
- **Folders with their own view**: give any folder its own mode and options (Pictures → 128 px icons and Movies → gallery are preconfigured). Finder keeps a folder's view in its parent folder's `.DS_Store` (under the folder's name) and in the folder's own; Winnow writes both, asks Finder to adopt the view on Apply (Automation permission, one prompt), and watches those files: whenever Finder rewrites one, Winnow puts it back, so a view change made in Finder lasts only until Finder next reads the folder. The parent folder and every sibling without its own view get a record for the defaults. Sweeps leave those files alone.
- **Reset the view when leaving a folder** (on by default): Finder keeps a window's view until the next folder has its own stored view, so a folder view carried one folder further and a view changed in Finder outlived its folder. Winnow polls Finder's windows twice a second and sets the default view whenever a window moves to a folder without its own, so a change made in Finder lasts exactly until you leave the folder. Needs the Automation permission (one prompt).
