...
| Action | Result |
|---|---|
| Hover a video, scroll right / left | Seek forward / back. Fast movement scrubs at the set speed (4 s per 100 px); slow, deliberate movement at a fifth of it, for frame-level control. No dead zone, no snapping. |
...
|---|---|---|
| Speed | 4 s per 100 px | The rate at fast wheel movement, whatever the video's length. 0.05 to 60 s per 100 px. |
...
- Wheel movement is converted to seconds and added to a logical position that tracks the wheel exactly, whatever the video is doing. Fractional pixels count; nothing is rounded or thresholded. The rate follows the wheel's velocity: the set speed applies at and above 1200 px/s (a fast spin or flick), a fifth of it at and below 150 px/s (a slow turn), with a smooth ramp between, so a flick of a free-spinning wheel covers ground and then, as the wheel slows, settles into frame-by-frame control.
