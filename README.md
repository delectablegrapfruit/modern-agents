...
**Library.** Home shows what you were reading, your reading goals and the library in numbers; the sidebar has All,
Finished, Books and PDFs and the collections you make (drag books onto them). Grid or list, sort by recent, title or
author, search by title, author and subject. Get Info edits title and author. Files come in through File ▸ Add to
...
Bookmarks · Notes popover, search within the book, an end-of-book card, and your place kept to the character. PDFs
open in the system's PDF view, tinted for the dark themes. The reading position, statistics (minutes per day,
streaks, books per year) and every annotation are files in `~/Library/Application Support/Books`.

**Wheel and keys.** A click of a mouse wheel — or a tilt of the wheel, or ⇧ + wheel — is exactly one page in
paginated mode; a two-finger swipe is one page, its inertial tail ignored. Arrow keys, space, Page Up/Down, Home and
End move too; ⌘] and ⌘[ jump chapters, ⌘+ and ⌘− change the text size. In vertical scrolling the wheel scrolls the
text.
...
- PDFs are shown by PDFKit: themes are colour filters, and there are no highlights in PDFs.
