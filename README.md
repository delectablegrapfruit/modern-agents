...
| Action | Result |
|---|---|
| Hover a video, scroll right / left | Seek forward / back at the set speed (4 s per 100 px). Keep scrolling in one direction for about three seconds and it ramps up to four times that; pause or reverse and it drops back. No dead zone, no snapping. |
...
|---|---|---|
| Speed | 4 s per 100 px | The base rate, whatever the video's length. 0.05 to 60 s per 100 px. |
...
- Wheel movement is converted to seconds and added to a logical position that tracks the wheel exactly, whatever the video is doing. Fractional pixels count; nothing is rounded or thresholded. Two modifiers apply. Velocity is a minor one, because scrolls are quick flicks and speed says little about intent: below 400 px/s the rate is 0.7 of the set speed, from 2400 px/s up it is the set speed, with a smooth ramp between. Persistence is the major one: scrolling kept up in one direction in quick succession (gaps under 350 ms) for 3 s ramps the rate up to 4x by 6 s; a pause or a reversal resets it, since you reverse when you overshoot, which is when you want fine control. The timeline shows the current multiplier when it is not 1.
