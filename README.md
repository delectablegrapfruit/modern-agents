...
- On youtube.com (and youtube-nocookie.com, youtu.be), which has this feature natively, the extension replaces it rather than doubling it: the `pointerdown`/`mousedown`/`mouseup` of a press on the video and the `keydown`/`keyup` of the space bar are stopped before the page's handlers see them, so one badge shows and one rate is set. The click a short press generates still reaches the page (it toggles playback as before), as do presses on the progress bar and the controls. As on the site itself, the space bar also reaches the main player from anywhere on the page: the playing video covering at least a fifth of the viewport, so inline previews do not count.
