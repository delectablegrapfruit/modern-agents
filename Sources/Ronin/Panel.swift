...
final class GameView: SKView {
    ...
    override func mouseDown(with event: NSEvent) {
        // Taking the keys lets the arrows cut too; the app you work in stays the active one.
        window?.makeKey()
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        windowDrag = nil
        if drag.moved { controller?.rememberPosition() } else if duelScene?.isCompact == true { controller?.setCompact(false) }
    }
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if duelScene?.key(event) != true { super.keyDown(with: event) }
    }
}
...
final class PanelController: NSObject {
    ...
    /// A wide, short strip: the lane is three and a third times as wide as it is tall.
    ...
    private static func initialFrame(size: NSSize) -> NSRect {
        ...
        return NSRect(x: visible.maxX - size.width - 20, y: visible.minY + 20, width: size.width, height: size.height)
    }
