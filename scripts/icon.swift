// Renders the app icon set: a plain rounded square with the sieve glyph.
import AppKit
...
func render(_ size: Int, scale: Int) {
    ...
    NSBezierPath(roundedRect: rect, xRadius: CGFloat(px) * 0.22, yRadius: CGFloat(px) * 0.22).fill()
    let config = NSImage.SymbolConfiguration(pointSize: CGFloat(px) * 0.5, weight: .light)
    if let symbol = NSImage(systemSymbolName: "line.3.horizontal.decrease", accessibilityDescription: nil)?.withSymbolConfiguration(config) {
