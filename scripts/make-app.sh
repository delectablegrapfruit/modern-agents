#!/usr/bin/env bash
# Builds Skirmish.app from the Swift package (macOS only).
#   scripts/make-app.sh [debug|release]
set -euo pipefail
cd "$(dirname "$0")/.."

CONFIG="${1:-release}"
APP="build/Skirmish.app"

swift build -c "$CONFIG" --product Skirmish
BINDIR="$(swift build -c "$CONFIG" --show-bin-path)"

rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "$BINDIR/Skirmish" "$APP/Contents/MacOS/Skirmish"
